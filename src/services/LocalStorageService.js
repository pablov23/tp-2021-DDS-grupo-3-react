...
const defineProperty = (prop, defaultKey = '', tag = '') => {
  ...
  module.exports[`set${capitalizedKey}`] = (val, key = defaultKey) =>
    setValue(`@@PERSON:${prop}${tag}${key}`, val);
  module.exports[`get${capitalizedKey}`] = (key = defaultKey) => getValue(`@@PERSON:${prop}${tag}${key}`);
  module.exports[`remove${capitalizedKey}`] = (key = defaultKey) => removeValue(`@@PERSON:${prop}${tag}${key}`);
};
